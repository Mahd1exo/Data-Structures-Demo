...
#include "task_stack.h"       // For task_stack_* functions
...
// Task to initialize the callbacks structure
...
// -------------------------------------------------------------------
// Public function: Start the server and return ServerResources
ServerResources start_server(const char** options) {
	...
	/*
	   ...
	   * We push tasks in reverse order of execution, so they
	   * pop off the stack in the correct order.
    */
	...
	// Return both the context and the callbacks pointer
	return resources;
}
