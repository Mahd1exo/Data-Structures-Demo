...
function HeapNode({ heap, index, level = 0 }) {
  if (index >= heap.length || heap[index] === null) {
    // Optionally render a null placeholder if you want to show empty spots:
    return (
      <motion.div
  ...
  return (
    <motion.div
      className="heap-node flex flex-col items-center relative"
      initial={{ opacity: 0, y: -10 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.3 }}
    >
      {/* Node value with level display */}
      <div className="text-xs text-gray-500 mb-1">Level {level}</div>
      <motion.div
        className="w-12 h-12 rounded-full border-2 border-black bg-green-500 text-white font-bold flex items-center justify-center shadow-sm"
      ...
      {((2 * index + 1) < heap.length || (2 * index + 2) < heap.length) && (
        <div className="mt-4 flex justify-center gap-4 relative">
          ...
          <div className="flex flex-col items-center">
            ...
            <HeapNode heap={heap} index={2 * index + 1} level={level + 1} />
          </div>
          <div className="flex flex-col items-center">
            <FaArrowDown className="text-gray-500" />
            <HeapNode heap={heap} index={2 * index + 2} level={level + 1} />
          </div>
        </div>
      )}
    </motion.div>
