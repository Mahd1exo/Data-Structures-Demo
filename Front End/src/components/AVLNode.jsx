...
function AVLNode({ node, level = 0 }) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="w-12 h-12 rounded-full border-2 border-black bg-orange-500 text-white font-bold flex items-center justify-center shadow-sm"
      ...
      {(node.left || node.right) && (
        <div className="mt-4 flex justify-center gap-4 relative">
          ...
          <div className="flex flex-col items-center">
            ...
            {node.left ? (
              <AVLNode node={node.left} level={level + 1} />
            ) : (
              <motion.div
                className="flex items-center justify-center w-12 h-12 rounded-full border-2 border-dashed border-gray-400 bg-gray-100 text-gray-500 font-bold shadow-sm"
                initial={{ scale: 0.8, opacity: 0.5 }}
          ...
          <div className="flex flex-col items-center">
            <FaArrowDown className="text-gray-500" />
            {node.right ? (
              <AVLNode node={node.right} level={level + 1} />
            ) : (
              <motion.div
                className="flex items-center justify-center w-12 h-12 rounded-full border-2 border-dashed border-gray-400 bg-gray-100 text-gray-500 font-bold shadow-sm"
                initial={{ scale: 0.8, opacity: 0.5 }}
